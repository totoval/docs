...
which Totoval is used. for example, Totoval's version is `v0.10.0`, Totoval framework's version
should also be `v0.10.0`.

## Totoval v0.10.x
Totoval `v0.10.x` upgrades the framework's underlying Totoval components to the `v0.10.x` series.

## Totoval v0.9.x
Totoval `v0.9.x` upgrades the framework's underlying Totoval components to the `v0.9.x` series.
### Compare
https://github.com/totoval/totoval/compare/v0.8.1-hotfix-3...v0.9.0
### Migrate to v0.9.x
#### 1. Code Replace
* global replace:

    * `gin.HandlerFunc` => `request.HandlerFunc`
    * `gin.Context` => `request.Context`
    * `gin.H` => `toto.V`
    * `logs.Field` => `toto.V`
* model replace:
    * `gin.Context` => `model.Context`

> the `request` package is "github.com/totoval/framework/request"  
> the `toto` package is "github.com/totoval/framework/helpers/toto"
> the `model` package is "github.com/totoval/framework/model"
